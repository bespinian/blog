...
Indeed, if we have a look in [Docker Hub](https://registry.hub.docker.com/r/bespinian/awesome-image/tags), which is the artifact store we use for our release process in this example, we can see that there are versions `1.0.0` and `2.0.0` of the `awesome-image` which have been released by our build process.
