...
We leveraged Vault’s built-in **Raft snapshot and restore** mechanism, which
allowed us to take a snapshot of the existing Vault data and restore it into a
new instance on the target AKS cluster. The migration was automated using a Bash
script that handled key steps such as Vault snapshot creation, restoration, and
unseal key migration.
...
- Check that the Managed Service Identity of the new Vault cluster has access to
  the unseal keys of both Vault instances
...
- Iterate through each follower pod and migrate using all three unseal keys:
...
traffic to the new Vault’s ingress.
